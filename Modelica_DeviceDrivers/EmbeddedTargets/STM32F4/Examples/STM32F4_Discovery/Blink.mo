...
model Blink
  ...
  inner Modelica_DeviceDrivers.EmbeddedTargets.STM32F4.Blocks.Microcontroller mcu(ahbPre = Modelica_DeviceDrivers.EmbeddedTargets.STM32F4.Types.AHBPre.DIV_1, apb1Pre = Modelica_DeviceDrivers.EmbeddedTargets.STM32F4.Types.APBPre.DIV_4, apb2Pre = Modelica_DeviceDrivers.EmbeddedTargets.STM32F4.Types.APBPre.DIV_2,clock = Modelica_DeviceDrivers.EmbeddedTargets.STM32F4.Types.Clock.HSE_PLL,desiredPeriod = 0.01, platform = Modelica_DeviceDrivers.EmbeddedTargets.STM32F4.Types.Platform.ARMCR4, pllM = 8, pllN = 336, pllP = Modelica_DeviceDrivers.EmbeddedTargets.STM32F4.Types.PLLP.DIV_2, pllQ = 7, preFlash = true, pwrRegVoltage = Modelica_DeviceDrivers.EmbeddedTargets.STM32F4.Types.PWRRegulatorVoltage.SCALE1)
  ...
  Functions.Digital.InitLed led1 = Functions.Digital.InitLed(handle = HALinit, led = Modelica_DeviceDrivers.EmbeddedTargets.STM32F4.Types.LED.LED4);
  Modelica.Blocks.Sources.BooleanExpression booleanExpression1(y = mod(time, 1) >= 0.5)  annotation(Placement(visible = true, transformation(origin = {-26, 2}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Functions.ClockConfig.Init clockConf = Functions.ClockConfig.Init(HALinit, mcu.clock, mcu.pllM, mcu.pllN, mcu.pllP,mcu.pllQ, mcu.ahbPre, mcu.apb1Pre, mcu.apb2Pre, mcu.pwrRegVoltage, mcu.overdrive, mcu.preFlash);
  Blocks.SynchronizeRealtime synchronizeRealtime1(clockConf = clockConf, timerFrequency = 10000, timerPeriod = 10000)  annotation(
